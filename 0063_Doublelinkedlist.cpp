...
void addNode()
{
	...
	cout << "\nEnter the roll number the student: ";
	...
	{ // step 2: insert the new node at the beginning
		...
		if (START != NULL && newNode->noMhs == START->noMhs)
		{
			cout << "\033[31mduplikate roll numbers not allowed\033[0m" << endl;
			return;
		}
		// if the list empty, make the new node the START
		newNode->next = START; // step 3: make the node node point to the first node
		if (START = newNode)
		{
	...
	else
	{
		...
		{							 // step 1.c: traverse the list to find the correct position
			previous = current;      // step 1.d: move the previous to the current to the current node
		...
		if (current != NULL)
		{
			current->prev = newNode; // step 6: make th previous field of the current node point to the new node
		}
